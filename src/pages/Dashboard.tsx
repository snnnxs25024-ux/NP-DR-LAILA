...
export function Dashboard() {
  ...
  return (
    <motion.div 
      ...
      initial="hidden"
      animate="show"
      className="p-4 md:p-6 space-y-4 md:space-y-6 h-full overflow-y-auto custom-scrollbar bg-slate-50/50"
    >
      {/* Header Section */}
...
function Widget({ title, value, icon: Icon, color, variants }: { title: string, value: string, icon: any, color: 'blue' | 'red' | 'green' | 'yellow', variants: any }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="text-xl md:text-3xl font-black text-slate-900 mb-1 tracking-tight">{value}</div>
