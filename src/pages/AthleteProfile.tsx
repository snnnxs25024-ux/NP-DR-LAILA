...
import { ArrowLeft, Calendar, Activity, Scale, Droplets, Moon, HeartPulse, Utensils, Stethoscope, ChevronRight, Share2, Download, Printer, ArrowRight, AlertTriangle, CheckCircle2, TrendingUp, ArrowUpRight, ArrowDownRight, X, Plus, User, MapPin, Ruler, Percent, Flame, Zap, Apple, Syringe } from 'lucide-react';
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-black text-slate-900 tracking-tight">{athlete.name}</h1>
                <span className={cn(
              ...
              <div className="flex items-center gap-3 mt-1.5">
                ...
                <span className="w-1 h-1 bg-slate-200 rounded-full"></span>
                <span className="text-[10px] font-black text-brand-red uppercase tracking-widest">Nutrition Analysis By DR LAILA</span>
              </div>
            </div>
          </div>
        </div>
...
function BodyVisualization({ athlete }: { athlete: any }) {
  const isMale = athlete.gender === 'Laki-laki';
  const [hoveredPart, setHoveredPart] = useState<string | null>(null);
  
  // Stable anatomical muscular system images from a reliable medical illustration source
  const maleAnatomy = "https://cdn.pixabay.com/photo/2012/04/13/13/41/human-32441_1280.png";
  const femaleAnatomy = "https://cdn.pixabay.com/photo/2012/04/13/13/41/human-32440_1280.png";

  const hotspots = [
    { id: 'head', label: 'Cognitive Status', value: 'Optimal', top: '8%', left: '50%', icon: Activity },
    { id: 'chest', label: 'Heart Rate', value: '72 bpm', top: '25%', left: '50%', icon: HeartPulse },
    { id: 'abs', label: 'Core Stability', value: 'Strong', top: '40%', left: '50%', icon: Zap },
    { id: 'arm-l', label: 'Left Bicep', value: `${(athlete.skeletalMuscleMass * 0.15).toFixed(1)} kg`, top: '35%', left: '30%', icon: Activity },
    { id: 'arm-r', label: 'Right Bicep', value: `${(athlete.skeletalMuscleMass * 0.15).toFixed(1)} kg`, top: '35%', left: '70%', icon: Activity },
    { id: 'leg-l', label: 'Left Quad', value: `${(athlete.skeletalMuscleMass * 0.25).toFixed(1)} kg`, top: '70%', left: '40%', icon: Activity },
    { id: 'leg-r', label: 'Right Quad', value: `${(athlete.skeletalMuscleMass * 0.25).toFixed(1)} kg`, top: '70%', left: '60%', icon: Activity },
  ];

  return (
    <div className="relative w-full max-w-[350px] h-[600px] flex items-center justify-center">
      {/* Anatomical Image */}
      <div className="relative h-full w-full flex items-center justify-center">
        <img 
          src={isMale ? maleAnatomy : femaleAnatomy} 
          alt="Anatomy" 
          className="h-full w-auto object-contain opacity-95 drop-shadow-[0_0_40px_rgba(0,0,0,0.15)]"
        ...
        {hotspots.map(spot => (
          ...
          >
            <div className={cn(
              "w-4 h-4 rounded-full border-2 border-white shadow-lg transition-all duration-300 cursor-pointer",
              hoveredPart === spot.id ? "bg-brand-red scale-125" : "bg-blue-500 hover:bg-brand-red"
            )}>
            ...
            >
              <div className="bg-white rounded-2xl p-4 shadow-2xl border border-slate-100 min-w-[140px] flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center">
      ...
      <div className="absolute -left-16 top-1/4 flex flex-col gap-4">
      ...
      <div className="absolute -right-16 bottom-1/4 flex flex-col gap-4">
