...
export const generateAssessmentPDF = (
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`${athlete.name}`, 38, 38);
  doc.text(`${athlete.category_name || '-'}`, 38, 44);
  doc.text(`${athlete.height} CM`, 38, 50);
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`${athlete.weight} KG`, 99, 38);
  doc.text(`${athlete.bf_caliper || 0} %`, 99, 44);
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`${athlete.target_weight} KG`, 158, 38);
  doc.text(`${athlete.target_body_fat} %`, 158, 44);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Laporan ini dihasilkan secara otomatis oleh Nutrition Report App. Dicetak pada: ${new Date().toLocaleString('id-ID')}`, 14, 285);
...
export const generateBulkAssessmentPDF = (
  reportsData: { 
    athlete: Athlete, 
    assessments: AssessmentEntry[], 
    title: string, 
  ...
  reportsData.forEach((data, index) => {
    ...
    const { athlete, assessments, title, diffUpdate, diffGlobal } = data;
    ...
    doc.setFont('helvetica', 'normal');
    // Ensure text bounds for very long names
    let shortName = athlete.name;
    if (shortName.length > 28) shortName = shortName.substring(0, 25) + '...';
    doc.text(`${shortName}`, 38, 38);
    doc.text(`${athlete.category_name || '-'}`, 38, 44);
    doc.text(`${athlete.height} CM`, 38, 50);
    ...
    doc.text('BB', 85, 38);
    doc.text(':', 101, 38);
    doc.text('Body Fat', 85, 44);
    doc.text(':', 101, 44);

    doc.setFont('helvetica', 'normal');
    doc.text(`${athlete.weight} KG`, 104, 38);
    doc.text(`${athlete.bf_caliper || 0} %`, 104, 44);

    // Column 3 (Right - Target)
    doc.setFont('helvetica', 'bold');
    doc.text('Target BB', 140, 38);
    doc.text(':', 160, 38);
    doc.text('Target BF', 140, 44);
    doc.text(':', 160, 44);

    doc.setFont('helvetica', 'normal');
    doc.text(`${athlete.target_weight} KG`, 163, 38);
    doc.text(`${athlete.target_body_fat} %`, 163, 44);
    ...
    // We can't automatically know doc.getNumberOfPages() inside the loop while adding pages dynamically easily for the footer, 
    // so we'll do the footer at the very end.
  });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(148, 163, 184);
    doc.text(`Laporan dihasilkan secara massal oleh Nutrition Report App. Dicetak pada: ${new Date().toLocaleString('id-ID')}`, 14, 285);
    doc.text(`Halaman ${i} dari ${pageCount}`, 196, 285, { align: 'right' });
  }
